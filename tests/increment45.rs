...
use agent_rig::{
    ...
    increment42_scene_json, increment43_scene_json, increment44_scene,
    increment44_scene_json, increment45_scene, increment45_scene_json,
    parse_scene, run_increment45, step_physics, Impulse, Joint, Light, Shape,
    DEFAULT_DT, INCREMENT44_STEPS, INCREMENT45_STEPS,
};
...
#[test]
fn increment45_does_not_mutate_prior_scene_json() {
    let prior_jsons: [(&str, &str); 27] = [
    ...
    for (name, json) in prior_jsons {
        assert!(
            !json.contains("motor_target_position"),
            "must not mutate increment {name} scene JSON with motor_target_position"
        );
    }

    let live44 = increment44_scene();
    let live45 = increment45_scene();
    assert_eq!(live44.lights.len(), live45.lights.len(), "increment45 must not add lights[] entries vs increment 44");
    assert_eq!(live44.bodies.len(), live45.bodies.len(), "increment45 must not add bodies vs increment 44");
    assert_eq!(live44.joints.len(), live45.joints.len(), "increment45 must not add joints vs increment 44");
    assert_eq!(live44.impulses.len(), live45.impulses.len());
    assert_eq!(live44.camera.position, live45.camera.position, "camera must stay increment-44");
    assert_eq!(live44.camera.look_at, live45.camera.look_at);

    let (_lim, v44, f44, pos44) = gate_hinge(&live44);
    assert!((v44 - 1.4).abs() < 1e-5, "increment44_scene gate motor_target_velocity should stay 1.4, got {v44}");
    assert!((f44 - 5.0).abs() < 1e-5);
    assert!(pos44.is_none(), "increment44_scene must not author motor_target_position, got {pos44:?}");
    let ser44 = serde_json::to_string(&live44).expect("serialize increment44_scene");
    assert!(
        !ser44.contains("motor_target_position"),
        "increment44_scene() must serialize without motor_target_position"
    );
}

#[test]
fn increment45_authors_position_motor_on_gate() {
    let parsed = parse_scene(increment45_scene_json()).expect("increment45 JSON should parse");
    let (limits, vel, force, pos) = gate_hinge(&parsed);
    assert!((limits[0] - 0.0).abs() < 1e-5 && (limits[1] - 1.15).abs() < 1e-5);
    assert!(vel.abs() < 1e-5, "increment45 gate motor_target_velocity should be 0, got {vel}");
    assert!((force - 5.0).abs() < 1e-5, "increment45 gate motor_max_force should be 5.0, got {force}");
    let target = pos.expect("increment45 JSON gate must author motor_target_position");
    assert!((target - 0.55).abs() < 1e-5, "increment45 JSON motor_target_position should be 0.55, got {target}");
    ...
    assert_eq!(parsed.camera.position, increment44_scene().camera.position);
    assert_eq!(parsed.camera.look_at, increment44_scene().camera.look_at);

    let live = increment45_scene();
    let (limits, vel, force, pos) = gate_hinge(&live);
    assert!((limits[0] - 0.0).abs() < 1e-5 && (limits[1] - 1.15).abs() < 1e-5);
    assert!(vel.abs() < 1e-5, "live increment45 gate motor_target_velocity should be 0, got {vel}");
    assert!((force - 5.0).abs() < 1e-5);
    let target = pos.expect("live increment45 gate must author motor_target_position");
    assert!((target - 0.55).abs() < 1e-5, "live motor_target_position should be 0.55, got {target}");
    assert_eq!(live.camera.position, [3.6, 2.35, 5.2]);
    assert_eq!(live.camera.look_at, [0.1, 0.38, 0.0]);
    assert_impulses(&live.impulses);

    let prior = increment44_scene();
    let (_lim, v44, _f44, pos44) = gate_hinge(&prior);
    assert!((v44 - 1.4).abs() < 1e-5);
    assert!(pos44.is_none(), "increment44_scene() must stay without motor_target_position");
}

#[test]
fn increment45_keeps_courtyard() {
    let scene = increment45_scene();
    let inc44 = increment44_scene();
    assert_eq!(scene.lights.len(), inc44.lights.len(), "no extra lights[] entries vs increment 44");
    assert_eq!(scene.lights.len(), 2, "no extra lights, got {}", scene.lights.len());
    assert_eq!(scene.bodies.len(), inc44.bodies.len(), "no new bodies vs increment 44");
    assert_eq!(scene.joints.len(), inc44.joints.len(), "no new joints vs increment 44");
...
#[test]
fn increment44_scene_stays_velocity_driven() {
    let scene = increment44_scene();
    let (_lim, vel, force, pos) = gate_hinge(&scene);
    assert!((vel - 1.4).abs() < 1e-5, "increment44_scene gate velocity should stay 1.4, got {vel}");
    assert!((force - 5.0).abs() < 1e-5);
    assert!(pos.is_none(), "increment44_scene must not author motor_target_position");
    let dump = step_physics(&scene, INCREMENT44_STEPS, DEFAULT_DT).expect("increment44 physics");
    let gate_j = dump
        .joints
        .iter()
    ...
    assert!(
        (angle - 1.15).abs() <= 0.2,
        "increment44 gate hinge angle should stay within 0.2 of 1.15, got {angle}"
    );
    assert!(
        gate_j.motor_target_position.is_none(),
        "increment44 dump must omit motor_target_position, got {:?}",
        gate_j.motor_target_position
    );
}

#[test]
fn increment45_gate_settles_at_target_not_limit() {
    let scene = increment45_scene();
    let dump = step_physics(&scene, INCREMENT45_STEPS, DEFAULT_DT).expect("physics");
...
#[test]
fn increment45_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment45.sh");
    assert!(script.is_file(), "scripts/increment45.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment45-threejs.sh");
    assert!(three.is_file(), "scripts/increment45-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment45-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment45(&out, INCREMENT45_STEPS, DEFAULT_DT, 200, 112).expect("run_increment45");
    ...
    let gtarget = gate_j["motor_target_position"]
        .as_f64()
        .expect("written dump gate hinge must record motor_target_position");
    assert!((gtarget - 0.55).abs() < 1e-5);
    let cork_state = v["bodies"]
        .as_array()
        .unwrap()
        .iter()
        .find(|b| b["id"] == "cork")
        .expect("dump should record the cork");
