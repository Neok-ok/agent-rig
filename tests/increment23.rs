...
use agent_rig::{
    ...
    increment23_scene_json, parse_scene, run_increment23, step_physics, Light, Shape, DEFAULT_DT,
    INCREMENT23_STEPS,
};
...
#[test]
fn increment23_does_not_mutate_prior_scene_json() {
    ...
    assert_eq!(
        s22.bodies.len(),
        7,
        "increment 22 scene JSON must stay 7 bodies, got {}",
        s22.bodies.len()
    );
    assert!(
        !increment18_scene_json().contains("sheen"),
        "must not mutate increment 18 scene JSON"
    );
    assert!(
        !increment20_scene_json().contains("sheen"),
        "must not mutate increment 20 scene JSON"
    );
    assert!(
        !increment21_scene_json().contains("sheen"),
        "must not mutate increment 21 scene JSON"
    );
    assert!(
        !increment22_scene_json().contains("sheen"),
        "must not mutate increment 22 scene JSON"
    );
    let bench22 = body_by_id(&s22, "bench");
    assert!(
        bench22.material.sheen <= 1e-6,
        "increment 22 bench must not gain sheen, got {}",
        bench22.material.sheen
    );
...
#[test]
fn increment23_bench_has_authorable_sheen() {
    let scene = parse_scene(increment23_scene_json()).expect("increment23 JSON should parse");
    let bench = body_by_id(&scene, "bench");
    assert!(
        matches!(&bench.shape, Shape::Mesh { path, .. } if path.contains("bench")),
        "bench should stay a mesh"
    );
    assert_eq!(bench.position, [1.35, 0.002, -0.15]);
    assert!((bench.material.albedo[0] - 0.32).abs() < 1e-4);
    assert!((bench.material.albedo[1] - 0.36).abs() < 1e-4);
    assert!((bench.material.albedo[2] - 0.40).abs() < 1e-4);
    assert!(
        bench.material.sheen > 0.5,
        "bench must author sheen, got {}",
        bench.material.sheen
    );
    assert!(
        bench.material.sheen_roughness > 0.0 && bench.material.sheen_roughness < 0.8,
        "bench sheen_roughness should be an authored mid-low value (velvet), got {}",
        bench.material.sheen_roughness
    );
    let c = bench.material.sheen_color;
    assert!(
        c[0] > 0.5 && c[1] < 0.35 && c[2] < 0.45,
        "bench sheen_color should be a saturated crimson, got {c:?}"
    );
}

#[test]
fn increment23_keeps_courtyard() {
    let scene = parse_scene(increment23_scene_json()).expect("increment23 JSON should parse");
    assert!(
        scene.bodies.len() >= 7,
    ...
    assert!(
        ...
        "keep the increment-22 courtyard including crate, bench, and pane"
    );
    ...
    for b in &scene.bodies {
        ...
        assert!(
            speed < 1e-6,
            "increment 23 is a still; body {} should have zero velocity, got {v:?}",
            b.id
        );
    }
}
...
fn contact_involving_gltf_body_after_step() {
    let scene = increment23_scene();
    let dump = step_physics(&scene, INCREMENT23_STEPS, DEFAULT_DT).expect("physics");
    let hit = dump
        .contacts
        .iter()
...
#[test]
fn increment23_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment23.sh");
    assert!(script.is_file(), "scripts/increment23.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment23-threejs.sh");
    assert!(three.is_file(), "scripts/increment23-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment23-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment23(&out, INCREMENT23_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment23");
    ...
    assert!(
        bench.material.sheen > 0.5,
        "written scene must author sheen on the bench, got {}",
        bench.material.sheen
    );
    assert!(
        bench.material.sheen_roughness > 0.0,
        "written scene must author sheen_roughness, got {}",
        bench.material.sheen_roughness
    );
    let c = bench.material.sheen_color;
    assert!(
        c[0] > 0.5 && c[1] < 0.35 && c[2] < 0.45,
        "written scene must author sheen_color, got {c:?}"
    );
    let ball = body_by_id(&scene, "ball");
    assert!(
        ball.material.clearcoat > 0.5,
