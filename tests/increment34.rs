...
use agent_rig::{
    ...
    increment33_scene_json, increment34_scene, increment34_scene_json, parse_scene,
    run_increment34, step_physics, Joint, Light, Shape, DEFAULT_DT, INCREMENT34_STEPS,
};
...
fn slider_json_has_motor(json: &str) -> bool {
    // Prior increment JSON already has hinge motor 4/8. Only the slider line
    // growing motor fields is the increment-34 mutation we must not see.
    for line in json.lines() {
        if line.contains("\"type\": \"slider\"")
            && (line.contains("motor_target_velocity") || line.contains("motor_max_force"))
        {
            return true;
        }
    }
...
#[test]
fn increment34_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !slider_json_has_motor(json),
            "must not mutate increment {name} scene JSON with slider motor fields"
        );
        assert!(
            !json.contains("\"motor_target_velocity\": -2"),
            "must not mutate increment {name} scene JSON with slider motor -2"
        );
        assert!(
            !json.contains("\"motor_max_force\": 6"),
            "must not mutate increment {name} scene JSON with slider motor force 6"
        );
    }

    let (_a, _b, _axis, _limits, v33, f33) = slider_of(&s33);
    assert_eq!(v33, 0.0, "increment33 JSON slider motor_target_velocity must stay 0");
    assert_eq!(f33, 0.0, "increment33 JSON slider motor_max_force must stay 0");

    let live33 = increment33_scene();
    let (_a, _b, _axis, _limits, v33l, f33l) = slider_of(&live33);
    assert_eq!(v33l, 0.0, "increment33_scene slider motor must stay 0");
    assert_eq!(f33l, 0.0, "increment33_scene slider motor_max_force must stay 0");
    let drawer33 = body_by_id(&live33, "drawer");
    assert_eq!(
        drawer33.linear_velocity,
        [0.0, 0.0, 2.5],
        "increment33 drawer must keep the +Z open velocity, got {:?}",
        drawer33.linear_velocity
    );
    let lantern33 = lantern_of(&live33);
    assert!(
        (lantern33.material.emissive_intensity - 16.0).abs() < 1e-5,
        "increment33 lantern must stay intensity 16, got {}",
        lantern33.material.emissive_intensity
    );
    assert_eq!(live33.triggers.len(), 1);
    assert_eq!(live33.triggers[0].id, "drawer_open");
    assert!(live33.bodies.iter().any(|b| b.id == "charm"));
    assert!(live33.joints.iter().any(|j| matches!(j, Joint::Ball { .. })));
    let (hv, hf) = hinge_motor(&live33);
    assert!(
        (hv - 4.0).abs() < 1e-5 && (hf - 8.0).abs() < 1e-5,
        "increment33 hinge motor must stay 4/8, got {hv}/{hf}"
    );
    assert_eq!(
        live33.lights.len(),
        increment34_scene().lights.len(),
        "increment34 must not add lights[] entries vs increment 33"
    );
    assert_eq!(
        live33.bodies.len(),
        increment34_scene().bodies.len(),
        "increment34 must not add bodies vs increment 33"
    );
    assert_eq!(
        live33.joints.len(),
        increment34_scene().joints.len(),
        "increment34 must not add joints vs increment 33"
    );
...
#[test]
fn increment34_slider_authors_motor() {
    let parsed = parse_scene(increment34_scene_json()).expect("increment34 JSON should parse");
    let (a, b, axis, limits, v, f) = slider_of(&parsed);
    assert_eq!(a, "crate");
    assert_eq!(b, "drawer");
    let alen = (axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]).sqrt();
    assert!(alen > 0.5, "parsed slider axis must be present, got {axis:?}");
    assert!(limits[1] > limits[0], "parsed slider limits must be a range");
    assert!(
        (v - (-2.0)).abs() < 1e-5,
        "parsed slider motor_target_velocity should be -2, got {v}"
    );
    assert!(
        (f - 6.0).abs() < 1e-5,
        "parsed slider motor_max_force should be 6, got {f}"
    );
    let drawer = body_by_id(&parsed, "drawer");
    assert_eq!(
        drawer.linear_velocity,
        [0.0, 0.0, 0.0],
        "parsed increment34 drawer must zero the +Z open velocity, got {:?}",
        drawer.linear_velocity
    );

    let live = increment34_scene();
    let (a, b, _axis, _limits, v, f) = slider_of(&live);
    assert_eq!(a, "crate");
    assert_eq!(b, "drawer");
    assert!(
        (v - (-2.0)).abs() < 1e-5,
        "live slider motor_target_velocity should be -2, got {v}"
    );
    assert!(
        (f - 6.0).abs() < 1e-5,
        "live slider motor_max_force should be 6, got {f}"
    );
    let drawer = body_by_id(&live, "drawer");
    assert_eq!(
        drawer.linear_velocity,
        [0.0, 0.0, 0.0],
        "live increment34 drawer must zero the +Z open velocity, got {:?}",
        drawer.linear_velocity
    );

    let scene33 = increment33_scene();
    let (_a, _b, _axis, _limits, v33, f33) = slider_of(&scene33);
    assert_eq!(v33, 0.0, "increment33 slider motor_target_velocity stays 0");
    assert_eq!(f33, 0.0, "increment33 slider motor_max_force stays 0");
}

#[test]
fn increment34_keeps_courtyard() {
    let scene = increment34_scene();
    assert!(
        scene.bodies.len() >= 10,
    ...
    let inc33 = increment33_scene();
    assert_eq!(
        scene.lights.len(),
        inc33.lights.len(),
        "no extra lights[] entries vs increment 33, got {} vs {}",
        scene.lights.len(),
        inc33.lights.len()
    );
    assert_eq!(
        scene.bodies.len(),
        inc33.bodies.len(),
        "no extra bodies vs increment 33"
    );
    assert_eq!(
        scene.joints.len(),
        inc33.joints.len(),
        "no extra joints vs increment 33"
    );
    ...
    assert!(
        has_bowl
            ...
            && has_drawer
            && has_charm,
        "keep the increment-33 courtyard including charm, drawer, lantern, crate, bench, pane"
    );
...
#[test]
fn increment34_dump_records_slider_motor_and_drawer_closed() {
    let scene = increment34_scene();
    let dump = step_physics(&scene, INCREMENT34_STEPS, DEFAULT_DT).expect("physics");
    assert!(
        dump.joints.len() >= 3,
    ...
    assert!(
        ...
        "closed drawer COM should sit near z=1.02 (not increment 33's ~1.375), got {:?}",
        drawer.position
    );
    // Trigger stays authored; overlap may be empty because the drawer left
    // the open sensor. That is the point: the motor closed it.
    let _ = &dump.overlaps;
}

#[test]
fn increment34_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment34.sh");
    assert!(script.is_file(), "scripts/increment34.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment34-threejs.sh");
    assert!(three.is_file(), "scripts/increment34-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment34-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment34(&out, INCREMENT34_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment34");
    ...
    assert!(
        dz < 1.15,
        "written dump drawer z should be closed (< 1.15, not ~1.375), got {dz}"
    );
