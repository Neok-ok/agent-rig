...
use agent_rig::{
    ...
    parse_scene, run_increment38, step_physics, Impulse, Joint, Light, Shape, DEFAULT_DT,
    INCREMENT38_STEPS,
};
...
fn assert_ball_impulse(impulses: &[Impulse]) {
    assert_eq!(impulses.len(), 1, "increment38 must author exactly one impulse, got {}", impulses.len());
...
#[test]
fn increment38_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !json.contains("\"impulses\""),
            "must not mutate increment {name} scene JSON with an impulses key"
        );
        assert!(
            parse_scene(json).expect(name).impulses.is_empty(),
            "increment {name} parsed scene must stay impulse-free"
        );
    }

    assert!(
        s37.impulses.is_empty(),
        "increment37 JSON must stay impulse-free, got {} impulses",
        s37.impulses.len()
    );
    assert!(s37.bodies.iter().any(|b| b.id == "lid"), "increment37 JSON must keep the lid");
    assert!(
        s37.joints.iter().any(|j| matches!(j, Joint::Fixed { .. })),
        "increment37 JSON must keep the fixed joint"
    );
    assert_eq!(s37.shapecasts.len(), 1, "increment37 JSON must keep drawer_sweep");
    assert_eq!(s37.shapecasts[0].id, "drawer_sweep");
    assert_eq!(s37.raycasts.len(), 1, "increment37 JSON must keep drawer_probe");
    assert_eq!(s37.raycasts[0].id, "drawer_probe");

    let live37 = increment37_scene();
    assert!(
        live37.impulses.is_empty(),
        "increment37_scene() must stay impulse-free, got {} impulses",
        live37.impulses.len()
    );
    assert!(live37.bodies.iter().any(|b| b.id == "lid"));
    assert!(live37.joints.iter().any(|j| matches!(j, Joint::Fixed { .. })));
    assert_eq!(live37.shapecasts.len(), 1);
    assert_eq!(live37.shapecasts[0].id, "drawer_sweep");
    assert_eq!(live37.raycasts.len(), 1);
    assert_eq!(live37.raycasts[0].id, "drawer_probe");
    let (_a, _b, _axis, _limits, v37, f37) = slider_of(&live37);
    assert!(
        (v37 - (-2.0)).abs() < 1e-5 && (f37 - 6.0).abs() < 1e-5,
        "increment37 slider motor must stay -2/6, got {v37}/{f37}"
    );
    let lantern37 = lantern_of(&live37);
    assert!(
        (lantern37.material.emissive_intensity - 16.0).abs() < 1e-5,
        "increment37 lantern must stay intensity 16, got {}",
        lantern37.material.emissive_intensity
    );
    assert_eq!(live37.triggers.len(), 1);
    assert_eq!(live37.triggers[0].id, "drawer_open");
    assert!(live37.bodies.iter().any(|b| b.id == "charm"));
    assert!(live37.joints.iter().any(|j| matches!(j, Joint::Ball { .. })));
    let (hv, hf) = hinge_motor(&live37);
    assert!(
        (hv - 4.0).abs() < 1e-5 && (hf - 8.0).abs() < 1e-5,
        "increment37 hinge motor must stay 4/8, got {hv}/{hf}"
    );
    let live38 = increment38_scene();
    assert_eq!(
        live37.lights.len(),
        live38.lights.len(),
        "increment38 must not add lights[] entries vs increment 37"
    );
    assert_eq!(
        live37.bodies.len(),
        live38.bodies.len(),
        "increment38 must not add bodies vs increment 37"
    );
    assert_eq!(
        live37.joints.len(),
        live38.joints.len(),
        "increment38 must not add joints vs increment 37"
    );
    assert_eq!(
        live37.impulses.len() + 1,
        live38.impulses.len(),
        "increment38 must add only the ball impulse vs increment 37"
    );
...
#[test]
fn increment38_authors_the_ball_impulse() {
    let parsed = parse_scene(increment38_scene_json()).expect("increment38 JSON should parse");
    assert_ball_impulse(&parsed.impulses);
    assert_eq!(
        parsed.camera.position,
        increment37_scene().camera.position,
        "parsed camera must stay increment-37"
    );
    assert_eq!(parsed.camera.look_at, increment37_scene().camera.look_at);

    let live = increment38_scene();
    assert_ball_impulse(&live.impulses);
    assert_eq!(live.camera.position, [3.6, 2.35, 5.2]);
    assert_eq!(live.camera.look_at, [0.1, 0.38, 0.0]);

    let prior = increment37_scene();
    assert!(
        prior.impulses.is_empty(),
        "increment37_scene() must stay impulse-free"
    );
}

#[test]
fn increment38_keeps_courtyard() {
    let scene = increment38_scene();
    assert!(
        scene.bodies.len() >= 11,
    ...
    let inc37 = increment37_scene();
    assert_eq!(
        scene.lights.len(),
        inc37.lights.len(),
        "no extra lights[] entries vs increment 37, got {} vs {}",
        scene.lights.len(),
        inc37.lights.len()
    );
    assert_eq!(
        scene.bodies.len(),
        inc37.bodies.len(),
        "no new bodies vs increment 37"
    );
    assert_eq!(
        scene.joints.len(),
        inc37.joints.len(),
        "no new joints vs increment 37"
    );
    ...
    assert!(
        has_bowl
            ...
            && has_charm
            && has_lid,
        "keep the increment-37 courtyard including lid, charm, drawer, lantern, crate, bench, pane"
    );
...
#[test]
fn increment38_ball_rolls_off_and_dump_records_impulse() {
    let scene = increment38_scene();
    let dump = step_physics(&scene, INCREMENT38_STEPS, DEFAULT_DT).expect("physics");
...
#[test]
fn increment38_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment38.sh");
    assert!(script.is_file(), "scripts/increment38.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment38-threejs.sh");
    assert!(three.is_file(), "scripts/increment38-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment38-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment38(&out, INCREMENT38_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment38");
