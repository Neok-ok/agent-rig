...
use agent_rig::{
    ...
    increment39_scene, increment39_scene_json, parse_scene, run_increment39, step_physics,
    Impulse, Joint, Light, Shape, DEFAULT_DT, INCREMENT39_STEPS,
};
...
fn assert_ball_impulse(impulses: &[Impulse]) {
    assert_eq!(impulses.len(), 1, "increment39 must keep exactly one impulse, got {}", impulses.len());
...
#[test]
fn increment39_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !json.contains("\"kinematic\""),
            "must not mutate increment {name} scene JSON with a kinematic key"
        );
        assert!(
            !json.contains("\"platform\""),
            "must not mutate increment {name} scene JSON with a platform body"
        );
        let parsed = parse_scene(json).expect(name);
        assert_no_platform_or_kinematic(&parsed, &format!("increment {name} JSON"));
    }
    ...
    assert_eq!(
        live38.lights.len(),
        live39.lights.len(),
        "increment39 must not add lights[] entries vs increment 38"
    );
    assert_eq!(
        live38.bodies.len() + 1,
        live39.bodies.len(),
        "increment39 must add only the platform body vs increment 38"
    );
    assert_eq!(
        live38.joints.len(),
        live39.joints.len(),
        "increment39 must not add joints vs increment 38"
    );
    assert_eq!(
        live38.impulses.len(),
        live39.impulses.len(),
        "increment39 must keep the ball impulse vs increment 38"
    );
    assert!(live39.bodies.iter().any(|b| b.id == "platform"));
    assert_eq!(
        live39.bodies.iter().filter(|b| b.kinematic).count(),
        1,
        "increment39 must author exactly one kinematic body"
    );
...
#[test]
fn increment39_authors_the_platform() {
    let parsed = parse_scene(increment39_scene_json()).expect("increment39 JSON should parse");
    let parsed_platform = parsed
        .bodies
        .iter()
        .find(|b| b.id == "platform")
        .expect("increment39 JSON must author platform");
    assert_platform(parsed_platform);
    assert_eq!(
        parsed.camera.position,
        increment38_scene().camera.position,
        "parsed camera must stay increment-38"
    );
    assert_eq!(parsed.camera.look_at, increment38_scene().camera.look_at);
    assert_ball_impulse(&parsed.impulses);

    let live = increment39_scene();
    let live_platform = live
        .bodies
        .iter()
        .find(|b| b.id == "platform")
        .expect("increment39_scene() must author platform");
    assert_platform(live_platform);
    assert_eq!(live.camera.position, [3.6, 2.35, 5.2]);
    assert_eq!(live.camera.look_at, [0.1, 0.38, 0.0]);
    assert_ball_impulse(&live.impulses);

    let prior = increment38_scene();
    assert_no_platform_or_kinematic(&prior, "increment38_scene()");
}

#[test]
fn increment39_keeps_courtyard() {
    let scene = increment39_scene();
    assert!(
        scene.bodies.len() >= 12,
        "scene must have courtyard + lantern + drawer + charm + lid + platform, got {}",
        scene.bodies.len()
    );
    assert_eq!(
        scene.triggers.len(),
    ...
    let inc38 = increment38_scene();
    assert_eq!(
        scene.lights.len(),
        inc38.lights.len(),
        "no extra lights[] entries vs increment 38, got {} vs {}",
        scene.lights.len(),
        inc38.lights.len()
    );
    assert_eq!(
        scene.bodies.len(),
        inc38.bodies.len() + 1,
        "only the platform body vs increment 38"
    );
    assert_eq!(
        scene.joints.len(),
        inc38.joints.len(),
        "no new joints vs increment 38"
    );
    ...
    assert!(
        has_bowl
            ...
            && has_lid
            && has_platform,
        "keep the increment-38 courtyard including lid, charm, drawer, lantern, crate, bench, pane"
    );
...
#[test]
fn increment39_platform_slides_and_dump_records_kinematic() {
    let scene = increment39_scene();
    let dump = step_physics(&scene, INCREMENT39_STEPS, DEFAULT_DT).expect("physics");
...
#[test]
fn increment39_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment39.sh");
    assert!(script.is_file(), "scripts/increment39.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment39-threejs.sh");
    assert!(three.is_file(), "scripts/increment39-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment39-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment39(&out, INCREMENT39_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment39");
    ...
    assert!(v["bodies"].as_array().unwrap().len() >= 12);
