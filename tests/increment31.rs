...
use agent_rig::{
    ...
    increment31_scene_json, parse_scene, run_increment31, step_physics, Joint, Light, Shape,
    DEFAULT_DT, INCREMENT31_STEPS,
};
...
#[test]
fn increment31_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !json.contains("charm"),
            "must not mutate increment {name} scene JSON with a charm"
        );
        assert!(
            !json.contains("\"type\": \"ball\""),
            "must not mutate increment {name} scene JSON with a ball joint"
        );
    }
    for (name, scene) in [
        ("18", s18),
        ...
        assert!(
            scene.bodies.iter().all(|b| b.id != "charm"),
            "increment {name} must not have a charm body"
        );
    }
    ...
    assert!(
        s28.bodies.iter().all(|b| b.id != "charm"),
        "increment28 must not have a charm body"
    );
    assert_eq!(s29.joints.len(), 2, "increment29 must keep hinge + slider");
    assert!(
        s29.joints.iter().all(|j| !matches!(j, Joint::Ball { .. })),
        "increment29 must not have a ball joint"
    );
    assert!(
        s29.bodies.iter().all(|b| b.id != "charm"),
        "increment29 must not have a charm body"
    );
    assert_eq!(s30.joints.len(), 2, "increment30 must keep hinge + slider");
    assert!(
        s30.joints.iter().all(|j| !matches!(j, Joint::Ball { .. })),
        "increment30 must not have a ball joint"
    );
    assert!(
        s30.bodies.iter().all(|b| b.id != "charm"),
        "increment30 must not have a charm body"
    );
    let (v30, f30) = hinge_motor(&s30);
    assert!(
        (v30 - 4.0).abs() < 1e-5 && (f30 - 8.0).abs() < 1e-5,
        "increment30 hinge motor must stay 4/8, got {v30}/{f30}"
    );
    let live30 = increment30_scene();
    assert!(
        live30.bodies.iter().all(|b| b.id != "charm"),
        "increment30_scene must not grow a charm"
    );
    assert!(
        live30.joints.iter().all(|j| !matches!(j, Joint::Ball { .. })),
        "increment30_scene must not grow a ball joint"
    );
    let live28 = increment28_scene();
    assert!(live28.bodies.iter().all(|b| b.id != "charm"));
    let live29 = increment29_scene();
    assert!(live29.bodies.iter().all(|b| b.id != "charm"));
...
#[test]
fn increment31_scene_has_charm_and_ball() {
    let parsed = parse_scene(increment31_scene_json()).expect("increment31 JSON should parse");
    let charm = body_by_id(&parsed, "charm");
    match charm.shape {
        Shape::Sphere { radius } => {
            assert!(
                (0.04..=0.08).contains(&radius),
                "charm sphere radius should be ~0.06, got {radius}"
            );
        }
        _ => panic!("charm should be a sphere, got {:?}", charm.shape),
    }
    assert!(charm.mass > 0.0, "charm must be dynamic, mass={}", charm.mass);
    let lantern = body_by_id(&parsed, "lantern");
    let dx = charm.position[0] - lantern.position[0];
    assert!(
        dx.abs() > 0.08,
        "charm must start offset in +X from the lantern (off the YZ swing plane), \
         charm={:?} lantern={:?}",
        charm.position,
        lantern.position
    );

    let balls: Vec<_> = parsed
        .joints
        .iter()
        .filter(|j| matches!(j, Joint::Ball { .. }))
        .collect();
    assert_eq!(balls.len(), 1, "expect one authored ball, got {}", balls.len());
    match balls[0] {
        Joint::Ball {
            body_a,
            body_b,
            ...
            assert_eq!(body_a, "lantern");
            assert_eq!(body_b, "charm");
            assert!(
                anchor.iter().any(|c| c.abs() > 1e-4),
                "ball anchor must be present, got {anchor:?}"
            );
        }
        _ => unreachable!(),
    }

    let live = increment31_scene();
    assert!(
        live.bodies.iter().any(|b| b.id == "charm"),
        "increment31_scene must include the charm"
    );
    let (a, b, anchor) = ball_of(&live);
    assert_eq!(a, "lantern");
    assert_eq!(b, "charm");
    assert!(
        anchor.iter().any(|c| c.abs() > 1e-4),
        "live ball anchor must be present, got {anchor:?}"
    );
    let (v, f) = hinge_motor(&live);
    assert!((v - 4.0).abs() < 1e-5, "keep hinge motor_target_velocity 4, got {v}");
    assert!((f - 8.0).abs() < 1e-5, "keep hinge motor_max_force 8, got {f}");
}

#[test]
fn increment31_keeps_courtyard() {
    let scene = increment31_scene();
    assert!(
        scene.bodies.len() >= 10,
    ...
    assert!(
        has_bowl
            ...
            && has_drawer
            && has_charm,
        "keep the increment-30 courtyard including drawer, lantern, crate, bench, pane, plus charm"
    );
...
#[test]
fn increment31_charm_hangs_and_dump_records_ball() {
    let scene = increment31_scene();
    let dump = step_physics(&scene, INCREMENT31_STEPS, DEFAULT_DT).expect("physics");
    assert!(
        dump.joints.len() >= 3,
    ...
    let charm = dump
        ...
        .iter()
        .find(|b| b.id == "charm")
        .expect("dump missing charm");
    assert_eq!(charm.collider, "ball", "charm collider should be a ball");
    let lantern = dump
        .bodies
        .iter()
        .find(|b| b.id == "lantern")
        .expect("dump missing lantern");
    let com = charm.position;
    let anchor = ballj.anchor;
    assert!(
        com[1] < anchor[1] - 0.04,
        "charm COM {com:?} must hang below socket anchor {anchor:?} \
         (com_y < anchor_y - 0.04)"
    );
    let dx = (com[0] - lantern.position[0]).abs();
    assert!(
        dx > 0.02,
        "charm must keep 2-axis freedom off the hinge swing plane: \
         |charm.x - lantern.x|={dx} charm={com:?} lantern={:?}",
        lantern.position
    );
}

#[test]
fn increment31_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment31.sh");
    assert!(script.is_file(), "scripts/increment31.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment31-threejs.sh");
    assert!(three.is_file(), "scripts/increment31-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment31-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment31(&out, INCREMENT31_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment31");
    ...
    let (sa, sb, _axis, limits) = slider_of(&scene);
    assert_eq!(sa, "crate");
    assert_eq!(sb, "drawer");
