...
use agent_rig::{
    ...
    increment41_scene, increment41_scene_json,
    parse_scene, run_increment41, step_physics,
    Impulse, Joint, Light, Shape, DEFAULT_DT, INCREMENT41_STEPS,
};
...
fn assert_gate(body: &agent_rig::Body) {
    ...
    assert!(
        !body.kinematic,
        "gate must not be kinematic, got kinematic={}",
        body.kinematic
    );
    match body.shape {
        Shape::Box { size } => {
    ...
    assert!(
        (body.mass - 0.5).abs() < 1e-5,
        "gate mass should be 0.5, got {}",
        body.mass
    );
    let a = body.material.albedo;
    assert!(
        (a[0] - 0.18).abs() < 1e-5 && (a[1] - 0.42).abs() < 1e-5 && (a[2] - 0.38).abs() < 1e-5,
        "gate albedo should be [0.18, 0.42, 0.38], got {a:?}"
    );
    assert!(
        (body.material.roughness - 0.45).abs() < 1e-5,
        "gate roughness should be 0.45, got {}",
        body.material.roughness
    );
    assert!(
        body.material.metallic.abs() < 1e-5,
        "gate metalness should be 0, got {}",
        body.material.metallic
    );
}
...
fn assert_no_gate(scene: &agent_rig::Scene, name: &str) {
    assert!(
        scene.bodies.iter().all(|b| b.id != "gate"),
        "{name} must stay gate-free"
    );
    for j in &scene.joints {
        if let Joint::Hinge { limits, body_a, body_b, .. } = j {
            assert!(
                limits.is_none(),
                "{name} hinge {body_a}–{body_b} must stay limit-free, got {limits:?}"
            );
        }
    }
}
...
#[test]
fn increment41_does_not_mutate_prior_scene_json() {
    ...
    let s40 = parse_scene(increment40_scene_json()).expect("inc40");
    ...
    assert_eq!(s40.bodies.len(), 13, "increment 40 scene JSON must stay 13 bodies");
    for (name, json) in [
        ...
        assert!(
            !json.contains("\"gate\""),
            "must not mutate increment {name} scene JSON with a gate body"
        );
        let parsed = parse_scene(json).expect(name);
        assert_no_gate(&parsed, &format!("increment {name} JSON"));
        for line in json.lines() {
            if line.contains("\"type\": \"hinge\"") {
                assert!(
                    !line.contains("\"limits\""),
                    "must not mutate increment {name} hinge JSON with limits: {line}"
                );
            }
        }
    }
    ...
    assert_no_gate(&live40, "increment40_scene()");
    assert_eq!(
        live40.joints.iter().filter(|j| matches!(j, Joint::Hinge { .. })).count(),
        1,
        "increment40 must keep exactly one hinge (pillar–lantern)"
    );
    ...
    assert_eq!(
        live40.lights.len(),
        live41.lights.len(),
        "increment41 must not add lights[] entries vs increment 40"
    );
    assert_eq!(
        live40.bodies.len() + 1,
        live41.bodies.len(),
        "increment41 must add only the gate body vs increment 40"
    );
    assert_eq!(
        live40.joints.len() + 1,
        live41.joints.len(),
        "increment41 must add only the ground–gate hinge vs increment 40"
    );
    assert_eq!(
        live40.impulses.len(),
        live41.impulses.len(),
        "increment41 must keep the ball impulse vs increment 40"
    );
    assert!(live41.bodies.iter().any(|b| b.id == "gate"));
    assert!(live41.bodies.iter().any(|b| b.id == "rider"));
    assert!(live41.bodies.iter().any(|b| b.id == "platform"));
    assert!(!body_by_id(&live41, "gate").kinematic, "gate must not be kinematic");
    let parsed40 = parse_scene(increment40_scene_json()).expect("inc40 live json");
    assert_no_gate(&parsed40, "increment40 JSON");
    assert_eq!(
        live40.bodies.len(),
        parsed40.bodies.len(),
        "increment40 JSON must stay unchanged vs increment40_scene()"
    );
...
#[test]
fn increment41_authors_the_gate() {
    let parsed = parse_scene(increment41_scene_json()).expect("increment41 JSON should parse");
    let parsed_gate = parsed
        .bodies
        .iter()
        .find(|b| b.id == "gate")
        .expect("increment41 JSON must author gate");
    assert_gate(parsed_gate);
    assert_rider(body_by_id(&parsed, "rider"));
    assert_platform(body_by_id(&parsed, "platform"));
    let (anchor, axis, limits, v, f) = gate_hinge(&parsed);
    assert!(
        (anchor[0] - 0.35).abs() < 1e-5
            && (anchor[1] - 0.04).abs() < 1e-5
            && (anchor[2] - 1.75).abs() < 1e-5,
        "gate hinge anchor should be [0.35, 0.04, 1.75], got {anchor:?}"
    );
    assert!(
        axis[0].abs() < 1e-5 && (axis[1] - 1.0).abs() < 1e-5 && axis[2].abs() < 1e-5,
        "gate hinge axis should be [0, 1, 0], got {axis:?}"
    );
    assert!(
        (limits[0] - 0.0).abs() < 1e-5 && (limits[1] - 1.15).abs() < 1e-5,
        "gate hinge limits should be [0.0, 1.15], got {limits:?}"
    );
    assert!((v - 1.4).abs() < 1e-5, "gate motor_target_velocity should be 1.4, got {v}");
    assert!((f - 5.0).abs() < 1e-5, "gate motor_max_force should be 5.0, got {f}");
    assert_eq!(
        parsed.camera.position,
        increment40_scene().camera.position,
        "parsed camera must stay increment-40"
    );
    assert_eq!(parsed.camera.look_at, increment40_scene().camera.look_at);
    assert_ball_impulse(&parsed.impulses);

    let live = increment41_scene();
    assert_gate(body_by_id(&live, "gate"));
    assert_rider(body_by_id(&live, "rider"));
    assert_platform(body_by_id(&live, "platform"));
    let (anchor, axis, limits, v, f) = gate_hinge(&live);
    assert!(
        (anchor[0] - 0.35).abs() < 1e-5
            && (anchor[1] - 0.04).abs() < 1e-5
            && (anchor[2] - 1.75).abs() < 1e-5,
        "live gate hinge anchor {anchor:?}"
    );
    assert!(
        axis[0].abs() < 1e-5 && (axis[1] - 1.0).abs() < 1e-5 && axis[2].abs() < 1e-5,
        "live gate hinge axis {axis:?}"
    );
    assert!(
        (limits[0] - 0.0).abs() < 1e-5 && (limits[1] - 1.15).abs() < 1e-5,
        "live gate hinge limits {limits:?}"
    );
    assert!((v - 1.4).abs() < 1e-5 && (f - 5.0).abs() < 1e-5, "live gate motor {v}/{f}");
    assert_eq!(live.camera.position, [3.6, 2.35, 5.2]);
    assert_eq!(live.camera.look_at, [0.1, 0.38, 0.0]);
    assert_ball_impulse(&live.impulses);

    let prior = increment40_scene();
    assert_no_gate(&prior, "increment40_scene()");
    assert_rider(body_by_id(&prior, "rider"));
    assert_platform(body_by_id(&prior, "platform"));
}

#[test]
fn increment41_keeps_courtyard() {
    let scene = increment41_scene();
    assert!(
        scene.bodies.len() >= 14,
        "scene must have courtyard + lantern + drawer + charm + lid + platform + rider + gate, got {}",
        scene.bodies.len()
    );
    assert_eq!(
        scene.triggers.len(),
    ...
    let inc40 = increment40_scene();
    assert_eq!(
        scene.lights.len(),
        inc40.lights.len(),
        "no extra lights[] entries vs increment 40, got {} vs {}",
        scene.lights.len(),
        inc40.lights.len()
    );
    assert_eq!(
        scene.bodies.len(),
        inc40.bodies.len() + 1,
        "only the gate body vs increment 40"
    );
    assert_eq!(
        scene.joints.len(),
        inc40.joints.len() + 1,
        "only the ground–gate hinge vs increment 40"
    );
    ...
    let has_gate = body_by_id(&scene, "gate").id == "gate";
    assert!(
        has_bowl
            && has_rock
            && has_ball
            ...
            && has_rider
            && has_gate,
        "keep the increment-40 courtyard including lid, charm, drawer, lantern, crate, bench, pane, platform, rider, plus gate"
    );
    ...
    let hinges: Vec<_> = scene
        ...
        .collect();
    assert_eq!(hinges.len(), 2, "keep the lantern hinge and add the gate hinge, got {}", hinges.len());
    let lantern_hinge = hinges.iter().find(|j| matches!(j, Joint::Hinge { body_a, body_b, .. } if body_a == "pillar" && body_b == "lantern"));
    assert!(lantern_hinge.is_some(), "keep the pillar–lantern hinge");
    match lantern_hinge.unwrap() {
        Joint::Hinge { limits, .. } => {
            assert!(limits.is_none(), "pillar–lantern hinge must stay limit-free, got {limits:?}");
        }
        _ => unreachable!(),
    }
...
#[test]
fn increment41_gate_yaws_to_limit_and_dump_records() {
    let scene = increment41_scene();
    let dump = step_physics(&scene, INCREMENT41_STEPS, DEFAULT_DT).expect("physics");
    ...
    assert!(
        !gate.kinematic,
        "dump gate must not be kinematic"
    );
    let authored_q = authored_gate.rotation_wxyz;
    let q = gate.rotation_wxyz;
    let quat_delta = (q[0] - authored_q[0]).abs()
        + (q[1] - authored_q[1]).abs()
        + (q[2] - authored_q[2]).abs()
        + (q[3] - authored_q[3]).abs();
    let pos_delta = (gate.position[0] - authored_gate.position[0]).abs()
        + (gate.position[2] - authored_gate.position[2]).abs();
    assert!(
        quat_delta > 0.15 || pos_delta > 0.05,
        "gate should have yawed vs authored pose, authored_q={authored_q:?} q={q:?} pos={:?}",
        gate.position
    );
    ...
    assert!(
        dump.joints.len() >= 5,
        "dump must record two hinges + slider + ball + fixed, got {}",
        dump.joints.len()
    );
    ...
    let hinge = dump
        .joints
        .iter()
        .find(|j| j.kind == "hinge" && j.body_a == "pillar" && j.body_b == "lantern")
        .expect("dump missing pillar–lantern hinge");
    assert_eq!(hinge.body_a, "pillar");
    assert_eq!(hinge.body_b, "lantern");
    assert!(
        hinge.limits.is_none(),
        "pillar–lantern hinge must stay limit-free in the dump, got {:?}",
        hinge.limits
    );
    ...
    assert!(
        (glim[0] - 0.0).abs() < 1e-5 && (glim[1] - 1.15).abs() < 1e-5,
        "dump gate hinge limits should be [0.0, 1.15], got {glim:?}"
    );
    let gmtv = gate_j
        .motor_target_velocity
        .expect("dump gate hinge must record motor_target_velocity");
    let gmmf = gate_j
        .motor_max_force
        .expect("dump gate hinge must record motor_max_force");
    assert!((gmtv - 1.4).abs() < 1e-5, "dump gate motor_target_velocity should be 1.4, got {gmtv}");
    assert!((gmmf - 5.0).abs() < 1e-5, "dump gate motor_max_force should be 5.0, got {gmmf}");
    ...
    assert!(
        angle <= 1.30,
        "gate hinge angle must not pass 1.30, got {angle}"
    );
    let mtv = hinge
        .motor_target_velocity
...
#[test]
fn increment41_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment41.sh");
    assert!(script.is_file(), "scripts/increment41.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment41-threejs.sh");
    assert!(three.is_file(), "scripts/increment41-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment41-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment41(&out, INCREMENT41_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment41");
    ...
    assert_rider(body_by_id(&scene, "rider"));
    assert_gate(body_by_id(&scene, "gate"));
    let (_anchor, _axis, glimits, gv, gf) = gate_hinge(&scene);
    assert!((glimits[0] - 0.0).abs() < 1e-5 && (glimits[1] - 1.15).abs() < 1e-5);
    assert!((gv - 1.4).abs() < 1e-5 && (gf - 5.0).abs() < 1e-5);
    ...
    assert!(v["bodies"].as_array().unwrap().len() >= 14);
    ...
    let hinge = joints
        .iter()
        .find(|j| j["kind"] == "hinge" && j["body_a"] == "pillar" && j["body_b"] == "lantern")
        .expect("dump joints must record the pillar–lantern hinge");
    assert_eq!(hinge["body_a"], "pillar");
    assert_eq!(hinge["body_b"], "lantern");
    assert!((hinge["motor_target_velocity"].as_f64().unwrap() - 4.0).abs() < 1e-5);
    assert!((hinge["motor_max_force"].as_f64().unwrap() - 8.0).abs() < 1e-5);
    assert!(hinge.get("limits").is_none() || hinge["limits"].is_null());
    let gate_j = joints
        .iter()
        .find(|j| j["kind"] == "hinge" && j["body_a"] == "ground" && j["body_b"] == "gate")
        .expect("dump joints must record the ground–gate hinge");
    let glim = gate_j["limits"].as_array().expect("written dump gate hinge must record limits");
    assert!((glim[0].as_f64().unwrap() - 0.0).abs() < 1e-5);
    assert!((glim[1].as_f64().unwrap() - 1.15).abs() < 1e-5);
    let gangle = gate_j["angle"].as_f64().expect("written dump gate hinge must record angle");
    assert!(
        (gangle - 1.15).abs() <= 0.2,
        "written dump gate angle should be within 0.2 of 1.15, got {gangle}"
    );
    assert!(gangle <= 1.30, "written dump gate angle must not pass 1.30, got {gangle}");
    let gate_state = v["bodies"]
        .as_array()
        .unwrap()
        .iter()
        .find(|b| b["id"] == "gate")
        .expect("dump should record the gate");
    assert_eq!(gate_state["kinematic"].as_bool().unwrap_or(false), false);
