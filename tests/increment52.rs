...
use agent_rig::{
    ...
    increment51_scene, increment51_scene_json, increment52_scene, increment52_scene_json,
    parse_scene, run_increment52, step_physics, Impulse, Joint, Light, Shape,
    DEFAULT_DT, INCREMENT51_STEPS, INCREMENT52_STEPS,
};
...
#[test]
fn increment52_does_not_mutate_prior_scene_json() {
    let prior_jsons: [(&str, &str); 34] = [
    ...
    for (name, json) in prior_jsons {
        assert!(
            !json.contains("\"follow\""),
            "must not mutate increment {name} scene JSON with follow"
        );
    }
    let live51 = increment51_scene();
    let live52 = increment52_scene();
    assert!(live51.camera.follow.is_none(), "increment51_scene must stay follow-free");
    assert!(live51.record_contact_events);
    assert!(live52.record_contact_events);
    assert_eq!(live51.lights.len(), live52.lights.len());
    assert_eq!(live51.bodies.len(), live52.bodies.len());
    assert_eq!(live51.joints.len(), live52.joints.len());
    assert_eq!(live51.pickups.len(), live52.pickups.len());
    assert_eq!(live51.triggers.len(), live52.triggers.len());
    assert_eq!(live51.camera.position, live52.camera.position);
    assert_eq!(live51.camera.look_at, live52.camera.look_at);
}

#[test]
fn increment52_scene_adds_follow() {
    let parsed = parse_scene(increment52_scene_json()).expect("increment52 JSON should parse");
    ...
    assert_no_extra_camera_keys(increment52_scene_json(), "increment52_scene_json");
    assert!(increment52_scene_json().contains("\"follow\""));
    let follow = parsed.camera.follow.as_ref().expect("increment52 camera must author follow");
    ...
    let live = increment52_scene();
    let f = live.camera.follow.as_ref().expect("live follow");
    assert_eq!(f.body, "walker");
    assert_eq!(f.offset, [1.20, 0.90, 1.50]);
    let prior = increment51_scene();
    assert!(prior.camera.follow.is_none());
    assert!(!increment51_scene_json().contains("\"follow\""));
}

#[test]
fn increment52_keeps_courtyard() {
    let scene = increment52_scene();
    let inc51 = increment51_scene();
    assert_eq!(scene.lights.len(), 2);
    assert_eq!(scene.bodies.len(), inc51.bodies.len());
    assert_eq!(scene.joints.len(), inc51.joints.len());
    assert_eq!(scene.pickups.len(), inc51.pickups.len());
...
#[test]
fn increment51_dump_has_no_camera() {
    let scene = increment51_scene();
    assert!(scene.camera.follow.is_none());
    let dump = step_physics(&scene, INCREMENT51_STEPS, DEFAULT_DT).expect("increment51 physics");
    assert!(dump.camera.is_none(), "increment51 dump must omit camera");
    let json = serde_json::to_value(&dump).expect("serialize increment51 dump");
    assert!(json.get("camera").is_none(), "increment51 dump JSON must omit camera");
...
#[test]
fn increment52_physics_follow_cam() {
    let scene = increment52_scene();
    let authored_x = body_by_id(&scene, "walker").position[0];
    let dump = step_physics(&scene, INCREMENT52_STEPS, DEFAULT_DT).expect("physics");
    ...
    assert!((cam.position[0] - 1.249).abs() < 0.08, "camera.x ~1.249, got {}", cam.position[0]);
    assert!((cam.position[1] - 1.084).abs() < 0.08, "camera.y ~1.084, got {}", cam.position[1]);
    assert!((cam.position[2] - 2.949).abs() < 0.08, "camera.z ~2.949, got {}", cam.position[2]);
    assert!((cam.look_at[0] - 0.049).abs() < 0.08, "look_at.x ~0.049, got {}", cam.look_at[0]);
    assert!((cam.look_at[1] - 0.334).abs() < 0.08, "look_at.y ~0.334, got {}", cam.look_at[1]);
    assert!((cam.look_at[2] - 1.449).abs() < 0.08, "look_at.z ~1.449, got {}", cam.look_at[2]);
    ...
    assert!(dump.picked_up.iter().any(|p| p.id == "token" && p.by == "walker" && p.at_step >= 30 && p.at_step <= 80));
    assert!(dump.spawned.iter().any(|s| s.id == "token" && s.at_step == 30));
...
#[test]
fn increment52_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment52.sh");
    assert!(script.is_file(), "scripts/increment52.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment52-threejs.sh");
    assert!(three.is_file(), "scripts/increment52-threejs.sh must exist");
    let out = PathBuf::from("target/test-increment52-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment52(&out, INCREMENT52_STEPS, DEFAULT_DT, 200, 112).expect("run_increment52");
    ...
    assert!((px - 1.249).abs() < 0.08 && (py - 1.084).abs() < 0.08 && (pz - 2.949).abs() < 0.08);
    let lx = cam["look_at"][0].as_f64().unwrap();
    let ly = cam["look_at"][1].as_f64().unwrap();
    let lz = cam["look_at"][2].as_f64().unwrap();
    assert!((lx - 0.049).abs() < 0.08 && (ly - 0.334).abs() < 0.08 && (lz - 1.449).abs() < 0.08);
