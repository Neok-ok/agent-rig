...
use agent_rig::{
    ...
    increment45_scene_json, increment46_scene_json, increment47_scene,
    increment47_scene_json, increment48_scene, increment48_scene_json, parse_scene,
    run_increment48, step_physics, Impulse, Joint, Light, Shape, DEFAULT_DT,
    INCREMENT47_STEPS, INCREMENT48_STEPS,
};
...
#[test]
fn increment48_does_not_mutate_prior_scene_json() {
    let prior_jsons: [(&str, &str); 30] = [
    ...
    for (name, json) in prior_jsons {
        assert!(
            ...
            "must not mutate increment {name} scene JSON with controller"
        );
        assert!(
            !json.contains("\"walker\""),
            "must not mutate increment {name} scene JSON with walker"
        );
    }
    ...
    assert!(
        live47.bodies.iter().all(|b| b.id != "walker" && b.controller.is_none()),
        "increment47_scene must stay walker-free and controller-free"
    );
    assert!(live47.record_contact_events);
    assert!(live48.record_contact_events, "increment48 must keep record_contact_events");
    assert_eq!(live47.lights.len(), live48.lights.len(), "increment48 must not add lights[] entries vs increment 47");
    assert_eq!(live47.bodies.len() + 1, live48.bodies.len(), "increment48 must add only walker vs increment 47");
    assert_eq!(live47.joints.len(), live48.joints.len(), "increment48 must not add joints vs increment 47");
    assert_eq!(live47.impulses.len(), live48.impulses.len());
    assert_eq!(live47.camera.position, live48.camera.position);
    assert_eq!(live47.camera.look_at, live48.camera.look_at);
    assert_eq!(live47.camera.fov_y_deg, live48.camera.fov_y_deg);
}

#[test]
fn increment48_scene_adds_walker_controller() {
    let parsed = parse_scene(increment48_scene_json()).expect("increment48 JSON should parse");
    ...
    assert_no_extra_camera_keys(increment48_scene_json(), "increment48_scene_json");
    assert!(increment48_scene_json().contains("\"controller\""));
    assert!(increment48_scene_json().contains("\"walker\""));
    ...
    assert!(vel.abs() < 1e-5, "increment48 gate motor_target_velocity should stay 0, got {vel}");
    assert!((force - 5.0).abs() < 1e-5, "increment48 gate motor_max_force should stay 5.0, got {force}");
    let target = pos.expect("increment48 JSON gate must keep motor_target_position");
    assert!((target - 0.55).abs() < 1e-5, "increment48 JSON motor_target_position should stay 0.55, got {target}");
    ...
    let target = pos.expect("live increment48 gate must keep motor_target_position");
    assert!((target - 0.55).abs() < 1e-5);
    assert_impulses(&live.impulses);

    let prior = increment47_scene();
    assert!(prior.bodies.iter().all(|b| b.id != "walker"));
    assert!(prior.bodies.iter().all(|b| b.controller.is_none()));
}

#[test]
fn increment48_keeps_courtyard() {
    let scene = increment48_scene();
    let inc47 = increment47_scene();
    assert_eq!(scene.lights.len(), inc47.lights.len(), "no extra lights[] entries vs increment 47");
    assert_eq!(scene.lights.len(), 2, "no extra lights, got {}", scene.lights.len());
    assert_eq!(scene.bodies.len(), inc47.bodies.len() + 1, "only walker vs increment 47");
    assert_eq!(scene.joints.len(), inc47.joints.len(), "no new joints vs increment 47");
...
#[test]
fn increment47_dump_has_no_controllers() {
    let scene = increment47_scene();
    assert!(scene.bodies.iter().all(|b| b.controller.is_none()));
    let dump = step_physics(&scene, INCREMENT47_STEPS, DEFAULT_DT).expect("increment47 physics");
    assert!(
        dump.controllers.is_empty(),
        "increment47 dump must stay without controllers, got {:?}",
        dump.controllers
    );
    let json = serde_json::to_value(&dump).expect("serialize increment47 dump");
    assert!(
        json.get("controllers").is_none(),
        "increment47 dump JSON must omit controllers when empty"
    );
    let scene_json = serde_json::to_string(&scene).expect("serialize increment47 scene");
    assert!(
        !scene_json.contains("\"controller\""),
        "increment47 scene JSON must omit controller when none"
    );
    assert!(
        !scene_json.contains("\"walker\""),
        "increment47 scene JSON must not contain walker"
    );
}

#[test]
fn increment48_physics_walks_and_stays_grounded() {
    let scene = increment48_scene();
    let authored_x = body_by_id(&scene, "walker").position[0];
    let dump = step_physics(&scene, INCREMENT48_STEPS, DEFAULT_DT).expect("physics");
    ...
    assert!(
        ...
        "increment48 dump must still record contact_events"
    );
    assert!(
...
#[test]
fn increment48_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment48.sh");
    assert!(script.is_file(), "scripts/increment48.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment48-threejs.sh");
    assert!(three.is_file(), "scripts/increment48-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment48-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment48(&out, INCREMENT48_STEPS, DEFAULT_DT, 200, 112).expect("run_increment48");
    ...
    assert!(scene_txt.contains("\"controller\""), "written scene must author controller");
    assert!(scene_txt.contains("\"walker\""), "written scene must author walker");
    ...
    assert!(v["bodies"].as_array().unwrap().len() >= 17);
