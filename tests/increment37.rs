...
use agent_rig::{
    ...
    increment35_scene_json, increment36_scene, increment36_scene_json, increment37_scene,
    increment37_scene_json, parse_scene, run_increment37, step_physics, Joint, Light, Shape,
    DEFAULT_DT, INCREMENT37_STEPS,
};
...
fn json_has_fixed(json: &str) -> bool {
    json.contains("\"type\": \"fixed\"") || json.contains("\"type\":\"fixed\"")
}

fn json_has_lid_body(json: &str) -> bool {
    json.contains("\"id\": \"lid\"") || json.contains("\"id\":\"lid\"")
}

#[test]
fn increment37_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !json_has_fixed(json),
            "must not mutate increment {name} scene JSON with a fixed joint"
        );
        assert!(
            !json_has_lid_body(json),
            "must not mutate increment {name} scene JSON with a lid body"
        );
    }

    assert!(
        s36.joints.iter().all(|j| !matches!(j, Joint::Fixed { .. })),
        "increment36 JSON must stay fixed-joint-free, got {} joints",
        s36.joints.len()
    );
    assert!(
        s36.bodies.iter().all(|b| b.id != "lid"),
        "increment36 JSON must stay lid-free"
    );
    assert_eq!(s36.shapecasts.len(), 1, "increment36 JSON must keep drawer_sweep");
    assert_eq!(s36.shapecasts[0].id, "drawer_sweep");
    assert_eq!(s36.raycasts.len(), 1, "increment36 JSON must keep drawer_probe");
    assert_eq!(s36.raycasts[0].id, "drawer_probe");

    let live36 = increment36_scene();
    assert!(
        live36.bodies.iter().all(|b| b.id != "lid"),
        "increment36_scene() must stay lid-free"
    );
    assert!(
        live36.joints.iter().all(|j| !matches!(j, Joint::Fixed { .. })),
        "increment36_scene() must stay fixed-joint-free, got {} joints",
        live36.joints.len()
    );
    assert_eq!(live36.shapecasts.len(), 1);
    assert_eq!(live36.shapecasts[0].id, "drawer_sweep");
    assert_eq!(live36.raycasts.len(), 1);
    assert_eq!(live36.raycasts[0].id, "drawer_probe");
    let (_a, _b, _axis, _limits, v36, f36) = slider_of(&live36);
    assert!(
        (v36 - (-2.0)).abs() < 1e-5 && (f36 - 6.0).abs() < 1e-5,
        "increment36 slider motor must stay -2/6, got {v36}/{f36}"
    );
    let lantern36 = lantern_of(&live36);
    assert!(
        (lantern36.material.emissive_intensity - 16.0).abs() < 1e-5,
        "increment36 lantern must stay intensity 16, got {}",
        lantern36.material.emissive_intensity
    );
    assert_eq!(live36.triggers.len(), 1);
    assert_eq!(live36.triggers[0].id, "drawer_open");
    assert!(live36.bodies.iter().any(|b| b.id == "charm"));
    assert!(live36.joints.iter().any(|j| matches!(j, Joint::Ball { .. })));
    let (hv, hf) = hinge_motor(&live36);
    assert!(
        (hv - 4.0).abs() < 1e-5 && (hf - 8.0).abs() < 1e-5,
        "increment36 hinge motor must stay 4/8, got {hv}/{hf}"
    );
    assert_eq!(
        live36.lights.len(),
        increment37_scene().lights.len(),
        "increment37 must not add lights[] entries vs increment 36"
    );
    assert_eq!(
        live36.bodies.len() + 1,
        increment37_scene().bodies.len(),
        "increment37 must add only the lid body vs increment 36"
    );
    assert_eq!(
        live36.joints.len() + 1,
        increment37_scene().joints.len(),
        "increment37 must add only the fixed joint vs increment 36"
    );
...
#[test]
fn increment37_has_lid_and_fixed_joint() {
    let parsed = parse_scene(increment37_scene_json()).expect("increment37 JSON should parse");
    let lid = body_by_id(&parsed, "lid");
    match &lid.shape {
        Shape::Box { size } => {
            assert!(
                (size[0] - 0.28).abs() < 0.02
                    && (size[1] - 0.04).abs() < 0.02
                    && (size[2] - 0.28).abs() < 0.02,
                "parsed lid box size should be ~[0.28, 0.04, 0.28], got {size:?}"
            );
        }
        other => panic!("parsed lid should be a box, got {other:?}"),
    }
    assert!(
        (lid.position[0] + 0.35).abs() < 0.05
            && (lid.position[1] - 0.28).abs() < 0.05
            && (lid.position[2] - 0.85).abs() < 0.05,
        "parsed lid should sit on the crate near [-0.35, 0.28, 0.85], got {:?}",
        lid.position
    );
    assert!(
        (lid.mass - 0.2).abs() < 0.05,
        "parsed lid mass should be ~0.2, got {}",
        lid.mass
    );
    let a = lid.material.albedo;
    assert!(
        a[0] > 0.3 && a[0] < 0.6 && a[1] > 0.15 && a[1] < 0.4 && a[2] < 0.25,
        "parsed lid should be wood-colored, got {a:?}"
    );
    assert!(
        (lid.material.roughness - 0.75).abs() < 0.1,
        "parsed lid roughness should be ~0.75, got {}",
        lid.material.roughness
    );

    let (a, b, anchor) = fixed_of(&parsed);
    assert_eq!(a, "crate");
    assert_eq!(b, "lid");
    assert!(
        (anchor[0] + 0.35).abs() < 0.05
            && (anchor[1] - 0.26).abs() < 0.05
            && (anchor[2] - 0.85).abs() < 0.05,
        "parsed fixed anchor should be the crate–lid interface near [-0.35, 0.26, 0.85], got {anchor:?}"
    );

    let live = increment37_scene();
    let lid = body_by_id(&live, "lid");
    match &lid.shape {
        Shape::Box { size } => {
            assert!(
                (size[0] - 0.28).abs() < 0.02
                    && (size[1] - 0.04).abs() < 0.02
                    && (size[2] - 0.28).abs() < 0.02,
                "live lid box size should be ~[0.28, 0.04, 0.28], got {size:?}"
            );
        }
        other => panic!("live lid should be a box, got {other:?}"),
    }
    assert!(
        (lid.position[0] + 0.35).abs() < 0.05
            && (lid.position[1] - 0.28).abs() < 0.05
            && (lid.position[2] - 0.85).abs() < 0.05,
        "live lid should sit on the crate near [-0.35, 0.28, 0.85], got {:?}",
        lid.position
    );
    assert!(
        (lid.mass - 0.2).abs() < 0.05,
        "live lid mass should be ~0.2, got {}",
        lid.mass
    );
    let (a, b, anchor) = fixed_of(&live);
    assert_eq!(a, "crate");
    assert_eq!(b, "lid");
    assert!(
        (anchor[0] + 0.35).abs() < 0.05
            && (anchor[1] - 0.26).abs() < 0.05
            && (anchor[2] - 0.85).abs() < 0.05,
        "live fixed anchor should be the crate–lid interface near [-0.35, 0.26, 0.85], got {anchor:?}"
    );

    let prior = increment36_scene();
    assert!(
        prior.bodies.iter().all(|b| b.id != "lid"),
        "increment36_scene() must stay lid-free"
    );
    assert!(
        prior.joints.iter().all(|j| !matches!(j, Joint::Fixed { .. })),
        "increment36_scene() must stay fixed-joint-free"
    );
}

#[test]
fn increment37_keeps_courtyard() {
    let scene = increment37_scene();
    assert!(
        scene.bodies.len() >= 11,
        "scene must have courtyard + lantern + drawer + charm + lid, got {}",
        scene.bodies.len()
    );
    assert_eq!(
        scene.triggers.len(),
    ...
    let inc36 = increment36_scene();
    assert_eq!(
        scene.lights.len(),
        inc36.lights.len(),
        "no extra lights[] entries vs increment 36, got {} vs {}",
        scene.lights.len(),
        inc36.lights.len()
    );
    assert_eq!(
        scene.bodies.len(),
        inc36.bodies.len() + 1,
        "only the lid body vs increment 36"
    );
    assert_eq!(
        scene.joints.len(),
        inc36.joints.len() + 1,
        "only the fixed joint vs increment 36"
    );
    ...
    assert!(
        has_bowl
            ...
            && has_charm
            && has_lid,
        "keep the increment-36 courtyard including charm, drawer, lantern, crate, bench, pane, plus lid"
    );
...
#[test]
fn increment37_lid_stays_seated_and_dump_records_fixed() {
    let scene = increment37_scene();
    let dump = step_physics(&scene, INCREMENT37_STEPS, DEFAULT_DT).expect("physics");
    assert!(
        dump.joints.len() >= 4,
...
#[test]
fn increment37_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment37.sh");
    assert!(script.is_file(), "scripts/increment37.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment37-threejs.sh");
    assert!(three.is_file(), "scripts/increment37-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment37-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment37(&out, INCREMENT37_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment37");
    ...
    assert!(v["bodies"].as_array().unwrap().len() >= 11);
