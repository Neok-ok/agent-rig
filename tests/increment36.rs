...
use agent_rig::{
    ...
    increment35_scene_json, increment36_scene, increment36_scene_json, parse_scene, run_increment36, step_physics, Joint, Light, Shape,
    DEFAULT_DT, INCREMENT36_STEPS,
};
...
fn json_has_shapecasts(json: &str) -> bool {
    json.contains("\"shapecasts\"") || json.contains("drawer_sweep")
}

#[test]
fn increment36_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !json_has_shapecasts(json),
            "must not mutate increment {name} scene JSON with shapecasts"
        );
    }

    assert!(
        s34.shapecasts.is_empty(),
        "increment34 JSON must stay shapecast-free, got {}",
        s34.shapecasts.len()
    );
    assert!(
        s35.shapecasts.is_empty(),
        "increment35 JSON must stay shapecast-free, got {}",
        s35.shapecasts.len()
    );
    assert_eq!(s35.raycasts.len(), 1, "increment35 JSON must keep drawer_probe");
    assert_eq!(s35.raycasts[0].id, "drawer_probe");
    let live35 = increment35_scene();
    assert!(
        live35.shapecasts.is_empty(),
        "increment35_scene() must stay shapecast-free, got {}",
        live35.shapecasts.len()
    );
    assert_eq!(live35.raycasts.len(), 1);
    assert_eq!(live35.raycasts[0].id, "drawer_probe");
    let (_a, _b, _axis, _limits, v34, f34) = slider_of(&live35);
    assert!(
        (v34 - (-2.0)).abs() < 1e-5 && (f34 - 6.0).abs() < 1e-5,
        "increment35 slider motor must stay -2/6, got {v34}/{f34}"
    );
    let lantern34 = lantern_of(&live35);
    assert!(
        (lantern34.material.emissive_intensity - 16.0).abs() < 1e-5,
        "increment35 lantern must stay intensity 16, got {}",
        lantern34.material.emissive_intensity
    );
    assert_eq!(live35.triggers.len(), 1);
    assert_eq!(live35.triggers[0].id, "drawer_open");
    assert!(live35.bodies.iter().any(|b| b.id == "charm"));
    assert!(live35.joints.iter().any(|j| matches!(j, Joint::Ball { .. })));
    let (hv, hf) = hinge_motor(&live35);
    assert!(
        (hv - 4.0).abs() < 1e-5 && (hf - 8.0).abs() < 1e-5,
        "increment35 hinge motor must stay 4/8, got {hv}/{hf}"
    );
    assert_eq!(
        live35.lights.len(),
        increment36_scene().lights.len(),
        "increment36 must not add lights[] entries vs increment 35"
    );
    assert_eq!(
        live35.bodies.len(),
        increment36_scene().bodies.len(),
        "increment36 must not add bodies vs increment 35"
    );
    assert_eq!(
        live35.joints.len(),
        increment36_scene().joints.len(),
        "increment36 must not add joints vs increment 35"
    );
...
#[test]
fn increment36_authors_drawer_sweep() {
    let parsed = parse_scene(increment36_scene_json()).expect("increment36 JSON should parse");
    assert_eq!(parsed.shapecasts.len(), 1, "parsed scene should author one shapecast");
    let sweep = &parsed.shapecasts[0];
    assert_eq!(sweep.id, "drawer_sweep");
    assert!(
        (sweep.origin[0] + 0.35).abs() < 0.05
            && (sweep.origin[1] - 0.55).abs() < 0.05
            && (sweep.origin[2] - 1.02).abs() < 0.05,
        "parsed origin should be near [-0.35, 0.55, 1.02], got {:?}",
        sweep.origin
    );
    assert!(
        sweep.direction[0].abs() < 1e-5
            && (sweep.direction[1] + 1.0).abs() < 1e-5
            && sweep.direction[2].abs() < 1e-5,
        "parsed direction should be [0, -1, 0], got {:?}",
        sweep.direction
    );
    match &sweep.shape {
        Shape::Box { size } => {
            assert!(
                (size[0] - 0.10).abs() < 1e-5
                    && (size[1] - 0.04).abs() < 1e-5
                    && (size[2] - 0.10).abs() < 1e-5,
                "parsed box size should be [0.10, 0.04, 0.10], got {size:?}"
            );
        }
        other => panic!("parsed shapecast should be a box, got {other:?}"),
    }
    assert!(
        (sweep.max_toi - 1.0).abs() < 0.05,
        "parsed max_toi should be 1.0, got {}",
        sweep.max_toi
    );
    assert_eq!(parsed.raycasts.len(), 1);
    assert_eq!(parsed.raycasts[0].id, "drawer_probe");

    let live = increment36_scene();
    assert_eq!(live.shapecasts.len(), 1, "live scene should author one shapecast");
    let sweep = &live.shapecasts[0];
    assert_eq!(sweep.id, "drawer_sweep");
    assert!(
        (sweep.origin[0] + 0.35).abs() < 0.05
            && (sweep.origin[1] - 0.55).abs() < 0.05
            && (sweep.origin[2] - 1.02).abs() < 0.05,
        "live origin should be near [-0.35, 0.55, 1.02], got {:?}",
        sweep.origin
    );
    assert!(
        sweep.direction[0].abs() < 1e-5
            && (sweep.direction[1] + 1.0).abs() < 1e-5
            && sweep.direction[2].abs() < 1e-5,
        "live direction should be [0, -1, 0], got {:?}",
        sweep.direction
    );
    match &sweep.shape {
        Shape::Box { size } => {
            assert!(
                (size[0] - 0.10).abs() < 1e-5
                    && (size[1] - 0.04).abs() < 1e-5
                    && (size[2] - 0.10).abs() < 1e-5,
                "live box size should be [0.10, 0.04, 0.10], got {size:?}"
            );
        }
        other => panic!("live shapecast should be a box, got {other:?}"),
    }
    assert!(
        (sweep.max_toi - 1.0).abs() < 0.05,
        "live max_toi should be 1.0, got {}",
        sweep.max_toi
    );
    assert_eq!(live.raycasts.len(), 1);
    assert_eq!(live.raycasts[0].id, "drawer_probe");

    let prior = increment35_scene();
    assert!(
        prior.shapecasts.is_empty(),
        "increment35_scene() must stay shapecast-free"
    );
    assert_eq!(prior.raycasts.len(), 1);
    assert_eq!(prior.raycasts[0].id, "drawer_probe");
}

#[test]
fn increment36_keeps_courtyard() {
    let scene = increment36_scene();
    assert!(
        scene.bodies.len() >= 10,
    ...
    let inc35 = increment35_scene();
    assert_eq!(
        scene.lights.len(),
        inc35.lights.len(),
        "no extra lights[] entries vs increment 35, got {} vs {}",
        scene.lights.len(),
        inc35.lights.len()
    );
    assert_eq!(
        scene.bodies.len(),
        inc35.bodies.len(),
        "no extra bodies vs increment 35"
    );
    assert_eq!(
        scene.joints.len(),
        inc35.joints.len(),
        "no extra joints vs increment 35"
    );
    ...
    assert!(
        has_bowl
            ...
            && has_drawer
            && has_charm,
        "keep the increment-35 courtyard including charm, drawer, lantern, crate, bench, pane"
    );
...
#[test]
fn increment36_dump_records_drawer_sweep_hit() {
    let scene = increment36_scene();
    let dump = step_physics(&scene, INCREMENT36_STEPS, DEFAULT_DT).expect("physics");
    assert!(
        dump.joints.len() >= 3,
...
#[test]
fn increment36_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment36.sh");
    assert!(script.is_file(), "scripts/increment36.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment36-threejs.sh");
    assert!(three.is_file(), "scripts/increment36-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment36-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment36(&out, INCREMENT36_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment36");
