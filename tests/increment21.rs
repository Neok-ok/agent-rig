...
use agent_rig::{
    ...
    increment21_scene_json, load_mesh, parse_scene, run_increment21, step_physics, Light, Shape,
    DEFAULT_DT, INCREMENT21_STEPS,
};
...
fn new_meshes_load() {
    ...
    let crate_path = manifest.join("meshes/crate.obj");
    let bench_path = manifest.join("meshes/bench.obj");
    assert!(crate_path.is_file(), "meshes/crate.obj must be checked in");
    assert!(bench_path.is_file(), "meshes/bench.obj must be checked in");
    let crate_mesh = load_mesh(&crate_path).expect("crate OBJ should load");
    let bench_mesh = load_mesh(&bench_path).expect("bench OBJ should load");
    assert!(
        crate_mesh.vertex_count() > 0 && crate_mesh.triangle_count() > 0,
        "crate empty"
    );
    assert!(
        bench_mesh.vertex_count() > 0 && bench_mesh.triangle_count() > 0,
        "bench empty"
    );
    assert!(
        crate_mesh.vertex_count() != bench_mesh.vertex_count()
            || crate_mesh.triangle_count() != bench_mesh.triangle_count(),
        "crate and bench should be distinct meshes"
    );
}

#[test]
fn increment21_does_not_mutate_prior_scene_json() {
    ...
    assert!(
        !increment20_scene_json().contains("crate.obj"),
        "must not mutate increment 20 scene JSON"
    );
    assert!(
        !increment18_scene_json().contains("crate.obj"),
...
#[test]
fn increment21_keeps_courtyard_and_adds_bodies() {
    let scene = parse_scene(increment21_scene_json()).expect("increment21 JSON should parse");
    assert!(
        scene.bodies.len() >= 7,
    ...
    assert!(
        ...
        "keep the increment-20 courtyard including the pane"
    );

    let crate_body = body_by_id(&scene, "crate");
    match &crate_body.shape {
        Shape::Mesh { path, collider } => {
            assert!(path.contains("crate"), "crate path {path}");
            let kind = format!("{collider:?}").to_lowercase();
            assert!(kind.contains("convex"), "crate collider={collider:?}");
        }
        other => panic!("crate should be a mesh, got {other:?}"),
    }
    let bench_body = body_by_id(&scene, "bench");
    match &bench_body.shape {
        Shape::Mesh { path, collider } => {
            assert!(path.contains("bench"), "bench path {path}");
            let kind = format!("{collider:?}").to_lowercase();
            assert!(
                kind.contains("convex") || kind.contains("trimesh"),
                "bench collider={collider:?}"
            );
        }
        other => panic!("bench should be a mesh, got {other:?}"),
    }
    ...
    for b in &scene.bodies {
        ...
        assert!(
            speed < 1e-6,
            "increment 21 is a still; body {} should have zero velocity, got {v:?}",
            b.id
        );
    }
}
...
fn contact_involving_gltf_body_after_step() {
    let scene = increment21_scene();
    let dump = step_physics(&scene, INCREMENT21_STEPS, DEFAULT_DT).expect("physics");
    let hit = dump
        .contacts
        .iter()
    ...
    for id in ["crate", "bench"] {
        let st = dump
            .bodies
            .iter()
            .find(|b| b.id == id)
            .unwrap_or_else(|| panic!("dump missing {id}"));
        assert!(
            st.collider == "convex_hull" || st.collider == "trimesh",
            "{id} collider={}",
            st.collider
        );
        let grounded = dump
            .contacts
            .iter()
            .any(|c| (c.body_a == id || c.body_b == id) && (c.body_a == "ground" || c.body_b == "ground"));
        assert!(
            grounded,
            "expected a ground contact for {id}, contacts={:?}",
            dump.contacts
        );
    }
}
...
#[test]
fn increment21_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment21.sh");
    assert!(script.is_file(), "scripts/increment21.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment21-threejs.sh");
    assert!(three.is_file(), "scripts/increment21-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment21-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment21(&out, INCREMENT21_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment21");
    ...
    for id in ["crate", "bench"] {
        let st = v["bodies"]
            ...
            .iter()
            .find(|b| b["id"] == id)
            .unwrap_or_else(|| panic!("dump should record {id}"));
        let col = st["collider"].as_str().unwrap_or("");
        assert!(
            col == "convex_hull" || col == "trimesh",
            "dump must record collider kind for {id}, got {col}"
        );
    }
