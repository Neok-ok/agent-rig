...
use agent_rig::{
    ...
    increment31_scene_json, increment32_scene, increment32_scene_json, parse_scene,
    run_increment32, step_physics, Joint, Light, Shape, DEFAULT_DT, INCREMENT32_STEPS,
};
...
#[test]
fn increment32_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !json.contains("drawer_open"),
            "must not mutate increment {name} scene JSON with a trigger"
        );
        assert!(
            !json.contains("\"triggers\""),
            "must not mutate increment {name} scene JSON with a triggers array"
        );
    }
    for (name, scene) in [
        ("18", s18),
        ...
        assert!(
            ...
            "increment {name} triggers must stay empty, got {}",
            scene.triggers.len()
        );
    }
    let live31 = increment31_scene();
    assert!(
        live31.triggers.is_empty(),
        "increment31_scene must not grow a trigger, got {}",
        live31.triggers.len()
    );
    assert!(
        live31.bodies.iter().any(|b| b.id == "charm"),
        "increment31_scene must keep the charm"
    );
    assert!(
        live31.joints.iter().any(|j| matches!(j, Joint::Ball { .. })),
        "increment31_scene must keep the ball joint"
    );
    let (v31, f31) = hinge_motor(&live31);
    assert!(
        (v31 - 4.0).abs() < 1e-5 && (f31 - 8.0).abs() < 1e-5,
        "increment31 hinge motor must stay 4/8, got {v31}/{f31}"
    );
    let live30 = increment30_scene();
    assert!(live30.triggers.is_empty(), "increment30_scene must stay trigger-free");
    let live28 = increment28_scene();
    assert!(live28.triggers.is_empty());
    let live29 = increment29_scene();
    assert!(live29.triggers.is_empty());
...
#[test]
fn increment32_scene_has_drawer_open_trigger() {
    let parsed = parse_scene(increment32_scene_json()).expect("increment32 JSON should parse");
    assert_eq!(
        parsed.triggers.len(),
        1,
        "expect one authored trigger, got {}",
        parsed.triggers.len()
    );
    let t = &parsed.triggers[0];
    assert_eq!(t.id, "drawer_open");
    match t.shape {
        Shape::Box { size } => {
            assert!(
                ...
                "trigger box size should be [0.30, 0.22, 0.28], got {size:?}"
            );
        }
        _ => panic!("trigger should be a box, got {:?}", t.shape),
    }
    assert_eq!(t.position, [-0.35, 0.10, 1.37]);

    let live = increment32_scene();
    assert_eq!(
        live.triggers.len(),
        1,
        "increment32_scene must include one trigger"
    );
    let t = &live.triggers[0];
    assert_eq!(t.id, "drawer_open");
    match t.shape {
        Shape::Box { size } => {
            assert!(
                (size[0] - 0.30).abs() < 1e-5
                    && (size[1] - 0.22).abs() < 1e-5
                    && (size[2] - 0.28).abs() < 1e-5,
                "live trigger box size should be [0.30, 0.22, 0.28], got {size:?}"
            );
        }
        _ => panic!("live trigger should be a box, got {:?}", t.shape),
    }
    assert_eq!(t.position, [-0.35, 0.10, 1.37]);

    // increment31 courtyard stays: charm + ball, hinge motor 4/8, drawer + slider
    assert!(live.bodies.iter().any(|b| b.id == "charm"));
    assert!(live.bodies.iter().any(|b| b.id == "drawer"));
    ...
    let (sa, sb, _axis, limits) = slider_of(&live);
    assert_eq!(sa, "crate");
    assert_eq!(sb, "drawer");
...
#[test]
fn increment32_keeps_courtyard() {
    let scene = increment32_scene();
    assert!(
        scene.bodies.len() >= 10,
    ...
    assert_eq!(
        scene.triggers.len(),
        1,
        "increment32 adds exactly one trigger, got {}",
        scene.triggers.len()
    );
    ...
    assert!(
        has_bowl
            ...
            && has_drawer
            && has_charm,
        "keep the increment-31 courtyard including charm, drawer, lantern, crate, bench, pane"
    );
...
#[test]
fn increment32_dump_records_drawer_open_overlap() {
    let scene = increment32_scene();
    let dump = step_physics(&scene, INCREMENT32_STEPS, DEFAULT_DT).expect("physics");
    assert!(
        dump.joints.len() >= 3,
    ...
    assert!(
        dump.overlaps.iter().any(|o| o.trigger == "drawer_open" && o.body == "drawer"),
        "dump overlaps must contain {{ trigger: drawer_open, body: drawer }}, got {:?}",
        dump.overlaps
    );
    // Drawer COM is already ~1.375 on Z after the slider opens.
    assert!(
        (drawer.position[2] - 1.375).abs() < 0.08,
        "open drawer COM should sit near z=1.375 overlapping the trigger, got {:?}",
        drawer.position
    );
}

#[test]
fn increment32_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment32.sh");
    assert!(script.is_file(), "scripts/increment32.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment32-threejs.sh");
    assert!(three.is_file(), "scripts/increment32-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment32-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment32(&out, INCREMENT32_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment32");
    ...
    let overlaps = v["overlaps"]
        .as_array()
        .expect("dump must have overlaps array");
    let hit = overlaps.iter().any(|o| {
        o["trigger"] == "drawer_open" && o["body"] == "drawer"
    });
    assert!(
        hit,
        "dump overlaps must contain {{ trigger: drawer_open, body: drawer }}, got {overlaps:?}"
    );
