...
use agent_rig::{
    ...
    increment33_scene_json, parse_scene, run_increment33, step_physics, Joint, Light, Shape,
    DEFAULT_DT, INCREMENT33_STEPS,
};
...
#[test]
fn increment33_does_not_mutate_prior_scene_json() {
    ...
    for (name, json) in [
        ...
        assert!(
            !json.contains("emissive_intensity"),
            "must not mutate increment {name} scene JSON with lantern emissive_intensity"
        );
        let has_orange = json.contains("1.0, 0.55, 0.12")
            || json.contains("1.0,0.55,0.12")
            || json.contains("[1.0, 0.55, 0.12]");
        assert!(
            !has_orange,
            "must not mutate increment {name} scene JSON with lantern orange emissive"
        );
    }
    for (name, scene) in [
        ("18", &s18),
        ...
        for b in &scene.bodies {
            assert!(
                b.material.emissive_intensity == 0.0,
                "increment {name} body {} must stay intensity 0, got {}",
                b.id,
                b.material.emissive_intensity
            );
        }
    }
    let live32 = increment32_scene();
    let lantern32 = lantern_of(&live32);
    assert!(
        lantern32.material.emissive_intensity == 0.0,
        "increment32_scene lantern must stay dark, got intensity {}",
        lantern32.material.emissive_intensity
    );
    let e = lantern32.material.emissive;
    assert!(
        e[0] == 0.0 && e[1] == 0.0 && e[2] == 0.0,
        "increment32_scene lantern must not grow orange emissive, got {e:?}"
    );
    assert_eq!(
        live32.triggers.len(),
        1,
        "increment32_scene must keep the drawer_open trigger"
    );
    assert_eq!(live32.triggers[0].id, "drawer_open");
    assert!(live32.bodies.iter().any(|b| b.id == "charm"));
    assert!(live32.joints.iter().any(|j| matches!(j, Joint::Ball { .. })));
    let (v32, f32v) = hinge_motor(&live32);
    assert!(
        (v32 - 4.0).abs() < 1e-5 && (f32v - 8.0).abs() < 1e-5,
        "increment32 hinge motor must stay 4/8, got {v32}/{f32v}"
    );
    assert_eq!(
        live32.lights.len(),
        increment33_scene().lights.len(),
        "increment33 must not add lights[] entries vs increment 32"
    );
...
#[test]
fn increment33_lantern_authors_emissive() {
    let parsed = parse_scene(increment33_scene_json()).expect("increment33 JSON should parse");
    let lantern = lantern_of(&parsed);
    let e = lantern.material.emissive;
    assert!(
        (e[0] - 1.0).abs() < 1e-5 && (e[1] - 0.55).abs() < 1e-5 && (e[2] - 0.12).abs() < 1e-5,
        "parsed lantern emissive should be [1.0, 0.55, 0.12], got {e:?}"
    );
    assert!(
        (lantern.material.emissive_intensity - 16.0).abs() < 1e-5,
        "parsed lantern emissive_intensity should be 16, got {}",
        lantern.material.emissive_intensity
    );

    let live = increment33_scene();
    let lantern = lantern_of(&live);
    let e = lantern.material.emissive;
    assert!(
        (e[0] - 1.0).abs() < 1e-5 && (e[1] - 0.55).abs() < 1e-5 && (e[2] - 0.12).abs() < 1e-5,
        "live lantern emissive should be [1.0, 0.55, 0.12], got {e:?}"
    );
    assert!(
        (lantern.material.emissive_intensity - 16.0).abs() < 1e-5,
        "live lantern emissive_intensity should be 16, got {}",
        lantern.material.emissive_intensity
    );

    let scene32 = increment32_scene();
    let lantern32 = lantern_of(&scene32);
    assert_eq!(
        lantern32.material.emissive_intensity, 0.0,
        "increment32 lantern stays dark"
    );
}

#[test]
fn increment33_keeps_courtyard() {
    let scene = increment33_scene();
    assert!(
        scene.bodies.len() >= 10,
    ...
    let inc32 = increment32_scene();
    assert_eq!(
        scene.lights.len(),
        inc32.lights.len(),
        "no extra lights[] entries vs increment 32, got {} vs {}",
        scene.lights.len(),
        inc32.lights.len()
    );
    assert_eq!(
        scene.bodies.len(),
        inc32.bodies.len(),
        "no extra bodies vs increment 32"
    );
    assert_eq!(
        scene.joints.len(),
        inc32.joints.len(),
        "no extra joints vs increment 32"
    );
    ...
    assert!(
        has_bowl
            ...
            && has_drawer
            && has_charm,
        "keep the increment-32 courtyard including charm, drawer, lantern, crate, bench, pane"
    );
...
#[test]
fn increment33_dump_records_drawer_open_overlap() {
    let scene = increment33_scene();
    let dump = step_physics(&scene, INCREMENT33_STEPS, DEFAULT_DT).expect("physics");
    assert!(
        dump.joints.len() >= 3,
...
#[test]
fn increment33_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment33.sh");
    assert!(script.is_file(), "scripts/increment33.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment33-threejs.sh");
    assert!(three.is_file(), "scripts/increment33-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment33-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment33(&out, INCREMENT33_STEPS, DEFAULT_DT, 200, 112)
        .expect("run_increment33");
