...
use agent_rig::{
    ...
    increment45_scene, increment45_scene_json, increment46_scene,
    increment46_scene_json, parse_scene, run_increment46, step_physics,
    Impulse, Joint, Light, Shape, DEFAULT_DT, INCREMENT45_STEPS, INCREMENT46_STEPS,
};
...
fn assert_one_camera(scene: &agent_rig::Scene) {
    // Scene has exactly one Camera field; no cameras[] array exists on the type.
    let _ = &scene.camera;
}
...
#[test]
fn increment46_does_not_mutate_prior_scene_json() {
    let prior_jsons: [(&str, &str); 28] = [
    ...
    for (name, json) in prior_jsons {
        assert!(
            json.contains("3.6"),
            "increment {name} scene JSON must keep the old camera (3.6)"
        );
        assert!(
            !json.contains("1.85"),
            "must not mutate increment {name} scene JSON with the increment-46 camera 1.85"
        );
        assert!(
            !json.contains("\"cameras\""),
            "must not mutate increment {name} scene JSON with cameras[]"
        );
    }

    let live45 = increment45_scene();
    let live46 = increment46_scene();
    assert_eq!(live45.lights.len(), live46.lights.len(), "increment46 must not add lights[] entries vs increment 45");
    assert_eq!(live45.bodies.len(), live46.bodies.len(), "increment46 must not add bodies vs increment 45");
    assert_eq!(live45.joints.len(), live46.joints.len(), "increment46 must not add joints vs increment 45");
    assert_eq!(live45.impulses.len(), live46.impulses.len());
    assert_eq!(live45.camera.position, [3.6, 2.35, 5.2], "increment45_scene camera must stay [3.6, 2.35, 5.2]");
    assert_eq!(live45.camera.look_at, [0.1, 0.38, 0.0]);
    assert!((live45.camera.fov_y_deg - 40.0).abs() < 1e-5);
}

#[test]
fn increment46_authors_reaimed_camera() {
    let parsed = parse_scene(increment46_scene_json()).expect("increment46 JSON should parse");
    ...
    assert_one_camera(&parsed);
    assert_no_extra_camera_keys(increment46_scene_json(), "INCREMENT46_SCENE_JSON");
    let (limits, vel, force, pos) = gate_hinge(&parsed);
    assert!((limits[0] - 0.0).abs() < 1e-5 && (limits[1] - 1.15).abs() < 1e-5);
    assert!(vel.abs() < 1e-5, "increment46 gate motor_target_velocity should stay 0, got {vel}");
    assert!((force - 5.0).abs() < 1e-5, "increment46 gate motor_max_force should stay 5.0, got {force}");
    let target = pos.expect("increment46 JSON gate must keep motor_target_position");
    assert!((target - 0.55).abs() < 1e-5, "increment46 JSON motor_target_position should stay 0.55, got {target}");
    ...
    assert_one_camera(&live);
    let (limits, vel, force, pos) = gate_hinge(&live);
    assert!((limits[0] - 0.0).abs() < 1e-5 && (limits[1] - 1.15).abs() < 1e-5);
    assert!(vel.abs() < 1e-5);
    assert!((force - 5.0).abs() < 1e-5);
    let target = pos.expect("live increment46 gate must keep motor_target_position");
    assert!((target - 0.55).abs() < 1e-5);
    assert_impulses(&live.impulses);

    let prior = increment45_scene();
    assert_eq!(prior.camera.position, [3.6, 2.35, 5.2]);
    assert_eq!(prior.camera.look_at, [0.1, 0.38, 0.0]);
    assert!((prior.camera.fov_y_deg - 40.0).abs() < 1e-5);
}

#[test]
fn increment46_keeps_courtyard() {
    let scene = increment46_scene();
    let inc45 = increment45_scene();
    assert_eq!(scene.lights.len(), inc45.lights.len(), "no extra lights[] entries vs increment 45");
    assert_eq!(scene.lights.len(), 2, "no extra lights, got {}", scene.lights.len());
    assert_eq!(scene.bodies.len(), inc45.bodies.len(), "no new bodies vs increment 45");
    assert_eq!(scene.joints.len(), inc45.joints.len(), "no new joints vs increment 45");
    ...
    assert_eq!(inc45.camera.position, [3.6, 2.35, 5.2]);
    assert_eq!(inc45.camera.look_at, [0.1, 0.38, 0.0]);
    let has_dir = scene.lights.iter().any(|l| matches!(l, Light::Directional { .. }));
    assert!(has_dir, "keep the directional");
}

#[test]
fn increment45_scene_stays_wide_courtyard() {
    let scene = increment45_scene();
    assert_eq!(scene.camera.position, [3.6, 2.35, 5.2]);
    assert_eq!(scene.camera.look_at, [0.1, 0.38, 0.0]);
    assert!((scene.camera.fov_y_deg - 40.0).abs() < 1e-5);
    let json = increment45_scene_json();
    assert!(json.contains("3.6"), "increment45 JSON must keep camera 3.6");
    assert!(!json.contains("1.85"), "increment45 JSON must not gain camera 1.85");
    assert_no_extra_camera_keys(json, "INCREMENT45_SCENE_JSON");
    let dump = step_physics(&scene, INCREMENT45_STEPS, DEFAULT_DT).expect("increment45 physics");
    let gate_j = dump
        .joints
        .iter()
    ...
    assert!(
        (angle - 0.55).abs() <= 0.15,
        "increment45 gate hinge angle should stay within 0.15 of 0.55, got {angle}"
    );
}

#[test]
fn increment46_physics_same_as_45() {
    let scene = increment46_scene();
    let dump = step_physics(&scene, INCREMENT46_STEPS, DEFAULT_DT).expect("physics");
...
#[test]
fn increment46_writes_scene_dump_and_png() {
    let script = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment46.sh");
    assert!(script.is_file(), "scripts/increment46.sh must exist");
    let three = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("scripts/increment46-threejs.sh");
    assert!(three.is_file(), "scripts/increment46-threejs.sh must exist");

    let out = PathBuf::from("target/test-increment46-artifacts");
    let _ = fs::remove_dir_all(&out);
    let paths = run_increment46(&out, INCREMENT46_STEPS, DEFAULT_DT, 200, 112).expect("run_increment46");
