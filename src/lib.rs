//! Agent-native scene + physics inspect + headless PNG (increments 1–61).
...
pub use physics::{simulate_trajectory, step_physics, step_catalog_scene, BrokenJoint, ContactEvent, ControllerState, DespawnRecord, DumpCamera, DroppedRecord, HeldRecord, PhysicsBodyState, PhysicsContact, PhysicsDump, PhysicsOverlap, PhysicsJoint, PickupRecord, SpawnRecord, StoppedRecord, Trajectory, TrajectoryFrame, TransitionRecord};
...
pub fn run_catalog_scene(
    ...
    let dump = step_catalog_scene(id, steps, dt)?;
    let dump_json = serde_json::to_string_pretty(&dump).map_err(|e| e.to_string())?;
    fs::write(&physics_path, dump_json).map_err(|e| format!("write physics: {e}"))?;
