...
enum Command {
    ...
    /// Increment 64: dump.lost when the win body is not delivered.
    Increment64 {
        #[arg(long, default_value = "artifacts/increment64")]
        out: PathBuf,
        #[arg(long, default_value_t = INCREMENT64_STEPS)]
        steps: u32,
        #[arg(long, default_value_t = FRAME_WIDTH)]
        width: u32,
        #[arg(long, default_value_t = FRAME_HEIGHT)]
...
fn main() {
    ...
    let result = match args.command {
        ...
        Some(Command::Increment64 {
            out,
            steps,
            width,
            height,
        }) => run_increment64(&out, steps, DEFAULT_DT, width, height).map(Some),
